...
for month in months:
    ...
    for v in range(5, 10):
        ...
        global velocities
        velocities=np.arange(-15, 10, v/10)
        for order in order_range:
            ...
            for counter in range(0, len(frames)):
                ...
                velocities, profile, profile_errors, alpha, continuum_waves, continuum_flux, no_line= LSD.LSD(wavelengths, flux, error, linelist, 'False', poly_ord, sn, order, run_name, velocities)
            ...
            phasess.append(phases)
            poptss.append(popts_new)
    plt.figure('Velocity Range Rvs')
    for v in range(0, 5):
        plt.scatter(phasess[v], poptss[v], label = 'np.arange(-15, 10, %s)'%(v/10))
    plt.legend()
    plt.show()
    # plt.show()
    plt.close('all')
