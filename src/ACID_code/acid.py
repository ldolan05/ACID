...
class Acid:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Also shown is where each parameter is stored (either in the :py:class:`Data` instance obtained from Acid.data) or in the :py:class:`Config` instance (stored as 
        Acid.config or Acid.data.config in memory). The :py:class:`Config` instance is for runtime settings and the :py:class:`Data` instance is for storing data and
        any calculations. 
        ...
        linelist_wl : :py:type:`Array1D`, optional
            Wavelengths of lines in the linelist (in Angstroms). Only necessary if linelist is not provided. 
            Must be same length as linelist_depths. If None, linelist must be provided (see above), by default None
        linelist_depths : :py:type:`Array1D`, optional
            Relative depths of lines in the linelist (between 0 and 1). Only necessary if linelist is not provided. 
