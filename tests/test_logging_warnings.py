...
import ACID_code
from ACID_code.diagnostics.warnings import ACIDDataWarning, ACIDWarning
...
def test_legacy_verbosity_logging_translation(verbose, expected):
    assert ACID_code.log_level_from_verbosity(verbose) == expected
...
def test_acid_warning_categories_are_public_and_filterable():
    assert issubclass(ACIDDataWarning, ACIDWarning)
    assert ACID_code.ACIDWarning is ACIDWarning

    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always", ACIDWarning)
        warnings.warn("problematic input", ACIDDataWarning)

    assert len(caught) == 1
    assert caught[0].category is ACIDDataWarning


def test_acid_warnings_default_to_once():
    ...
    with warnings.catch_warnings():
        ...
        assert any(
            action == "once" and category is ACIDWarning
